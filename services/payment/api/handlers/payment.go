...
func (s *PaymentService) TickerPrice(c *gin.Context) {
	var data binance.TickerPriceRequest
	if err := c.BindJSON(&data); err != nil {
		logger.Logger.Error("Invalid input data", zap.Error(err))
		c.JSON(errors.GetHTTPStatus(errors.TypeInvalidInput), gin.H{"status": "error", "message": errors.TypeInvalidInput})
		return
	...
	resp, err := s.payment.TickerPrice(data)
...
func (s *PaymentService) Order(c *gin.Context) {
	var data binance.BinanceOrderRequest
	if err := c.BindJSON(&data); err != nil {
		logger.Logger.Error("Invalid input data", zap.Error(err))
		c.JSON(errors.GetHTTPStatus(errors.TypeInvalidInput), gin.H{"status": "error", "message": errors.TypeInvalidInput})
		return
	...
	resp, err := s.payment.Order(c, data)
